...
public class Argo2ModelloMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		ProjectFilePersister persister = PersistenceManager.getInstance()
				.getPersisterFromFileName(sourceModel.getAbsolutePath());

		Project p = null;
		try {
			p = persister.doLoad(sourceModel);
		} catch (Exception e) {
			throw new MojoExecutionException("Cannot load model '"
					+ sourceModel.getAbsolutePath() + "': " + e.getMessage(), e);
		}
		...
		while (it.hasNext()) {
			...
			if (!seenPkg) {
				seenPkg = true;
				addElement(pkgDef, "value", facade.getName(facade
						.getNamespace(inf)));
			}
		...
		try {
			...
			throw new MojoExecutionException("Cannot write model '"
					+ destinationModel.getAbsolutePath() + "': "
					+ e.getMessage(), e);
		}
	}
	...
	public void addFields(Object clazz, Element elemClazz) {
		...
		if (!fieldsUml.isEmpty()) {
			...
			while (jt.hasNext()) {
				...
				log.info("Add " + facade.getName(attr) + " with "
						+ facade.getName(facade.getType(attr)));
				if (facade.getType(attr) != null)
					addElement(elemField, "type", facade.getName(facade
							.getType(attr)));
				// if not default case
	...
	public void addInterfaces(Object cls, Element elemClazz) {
        ...
        if (!realizations.isEmpty()) {
            ...
            while (clsEnum.hasNext()) {
                ...
                String infName = Model.getFacade().getName(inter);
                String infPkg = Model.getFacade().getName(Model.getFacade().getNamespace(inter));
                addElement(interfaces, "interface", infPkg + '.' + infName);
            }
        }
	}
	...
	public void addAssociations(Object clazz, Element elemClazz) {
		...
		if (!ends.isEmpty()) {
			...
			for (Object associationEnd : ends) {
				...
				String typeName = facade.getName(facade
						.getType(otherAssociationEnd));
				if ("".equals(endName) || endName == null) {
					endName = typeName;
				}
				String multiplicity = facade.getName(facade
						.getMultiplicity(otherAssociationEnd));
				Element elemField = addElement(fields, "field");
				...
				if (elemField.getChild("annotations") != null) {
					Element annotations = (Element) elemField.getChild(
							"annotations").clone();
